...
//	@Query("SELECT e FROM ExamAdmin e JOIN FETCH e.(new com.app.pojos.Client(c.id, c.name, c.email, c.mobile, c.addressLine1, c.addressLine2,c.subscription, c.examCount)) c WHERE e.email =?1 ")
	...
	@Query("SELECT new com.examportal.user.dto.ExamAdminDTO(e.id, e.name, e.email, e.mobile, e.role, e.accStatus,e.dateStamp, e.addressLine1, e.addressLine2, e.address, e.department, e.client.id, e.dob, e.gender) FROM ExamAdmin e INNER JOIN e.client c WHERE e.email =?1")
