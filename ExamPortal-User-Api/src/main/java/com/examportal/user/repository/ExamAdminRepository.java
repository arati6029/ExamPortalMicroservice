...
public interface ExamAdminRepository extends JpaRepository<ExamAdmin, Long>{
	@Query("SELECT e FROM ExamAdmin e JOIN FETCH e.client c WHERE e.email =?1 AND e.password =?2")
	Optional<ExamAdmin> findByEmailAndPassword(String email,String password);
	
	@Query("SELECT e FROM ExamAdmin e JOIN FETCH e.client c WHERE e.id =?1")
	...
	@Query("SELECT new com.examportal.user.dto.ExamAdminListDTO(e.id, e.name, e.email, e.department, e.accStatus) FROM ExamAdmin e WHERE e.client.id=?1")
...
//	@Query("SELECT e FROM ExamAdmin e JOIN FETCH e.(new com.app.pojos.Client(c.id, c.name, c.email, c.mobile, c.addressLine1, c.addressLine2,c.subscription, c.examCount)) c WHERE e.email =?1 ")
	...
	@Query("SELECT new com.examportal.user.dto.ExamAdminDTO(e.id, e.name, e.email, e.mobile, e.role, e.accStatus,e.dateStamp, e.addressLine1, e.addressLine2, e.address.pinCode, e.department, e.client.id, e.dob, e.gender) FROM ExamAdmin e INNER JOIN e.client c WHERE e.email =?1")
